...
import { Check, Zap, Star, Rocket, Sparkles, X, HelpCircle, CreditCard, Shield, Clock, CheckCircle, AlertCircle, Banknote, Copy, History, RefreshCw, Layers } from 'lucide-react';
import { useLanguage } from '../../context/LanguageContext';
import { getDefaultPackageCatalog, getPackageCatalog, getWallet, getEmployerSubscriptions } from '../../services/packageCatalogService';
...
const ActivePackagesWrap = styled.div`
  margin-bottom: 28px;
`;

const ActivePackagesGrid = styled.div`
  display: grid;
...
const ActivePackageCard = styled(motion.div)`
...
const Subscription = () => {
  ...
  const loadMySubscriptions = React.useCallback(async () => {
    try {
      setIsLoadingSubscriptions(true);
      const { employerId } = await getEmployerInfo();
      if (employerId && employerId !== 'unknown') {
        const subs = await getEmployerSubscriptions(employerId);
        setMySubscriptions(subs);
      }
    } catch (err) {
      console.error('Error loading employer subscriptions:', err);
    } finally {
  ...
  <DashboardLayout role="employer" showSearch={false} key={lang}>
    ...
    >
      <Inner>
        ...
        {/* Active Subscriptions Banner/Cards */}
        {(() => {
            ...
            <ActivePackagesWrap>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <span style={{ fontSize: '15px', fontWeight: '800', color: '#1e293b' }}>
                  {vi ? 'Gói dịch vụ đang kích hoạt của bạn' : 'Your Active Packages'}
                </span>
              </div>
              <ActivePackagesGrid>
                {activeSubs.map((sub, idx) => (
                  <ActivePackageCard
                    key={sub.subscriptionId || idx}
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '14px' }}>
                      ...
                      <ActiveBadge>
                        <CheckCircle size={13} /> {vi ? 'Đang hoạt động' : 'Active'}
                      </ActiveBadge>
                    </div>

                    <div style={{ fontSize: '13px', opacity: 0.9, marginBottom: '6px' }}>
                      <strong>{vi ? 'Thời hạn:' : 'Duration:'}</strong> {sub.duration}
                    </div>

                    <div style={{ fontSize: '12.5px', opacity: 0.85, display: 'flex', alignItems: 'center', gap: '5px' }}>
                      <Clock size={13} />
                      {vi ? 'Hết hạn lúc:' : 'Expires on:'} {sub.endDate ? new Date(sub.endDate).toLocaleDateString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' }) : 'N/A'}
                    </div>
                  </ActivePackageCard>
                ))}
              </ActivePackagesGrid>
        ...
        <HistoryContainer>
          <HistoryHeader>
            <h3>
              ...
              {vi ? 'Lịch sử mua gói dịch vụ' : 'Package Purchase History'}
            </h3>
            <RefreshBtn onClick={loadMySubscriptions} disabled={isLoadingSubscriptions}>
              <RefreshCw size={14} className={isLoadingSubscriptions ? 'animate-spin' : ''} />
              {vi ? (isLoadingSubscriptions ? 'Đang tải...' : 'Làm mới') : (isLoadingSubscriptions ? 'Loading...' : 'Refresh')}
            </RefreshBtn>
          </HistoryHeader>
          ...
          <TableResponsive>
            <HistoryTable>
              <thead>
                <tr>
                  <th>{vi ? 'Gói dịch vụ' : 'Package'}</th>
                  <th>{vi ? 'Thời hạn' : 'Duration'}</th>
                  <th>{vi ? 'Số tiền' : 'Amount'}</th>
                  <th>{vi ? 'Ngày mua' : 'Purchase Date'}</th>
                  <th>{vi ? 'Ngày hết hạn' : 'Expiry Date'}</th>
                  <th>{vi ? 'Thanh toán' : 'Payment'}</th>
                  <th style={{ textAlign: 'center' }}>{vi ? 'Trạng thái' : 'Status'}</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    return (
                      <tr key={sub.subscriptionId || idx}>
                        ...
                        <td style={{ fontWeight: '700', color: '#059669' }}>
                          {sub.price ? `${Number(sub.price).toLocaleString('vi-VN')} VND` : '—'}
                        </td>
                        <td>
                          {sub.purchaseDate
                            ? new Date(sub.purchaseDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })
                            : (sub.createdAt ? new Date(sub.createdAt).toLocaleDateString('vi-VN') : '—')}
                        </td>
                        <td>
                          {sub.endDate
                            ? new Date(sub.endDate).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })
                            : '—'}
                        </td>
                        <td>
                          <span style={{ textTransform: 'capitalize' }}>
                            {sub.paymentMethod === 'wallet' ? (vi ? 'Ví OpPo' : 'Wallet') : (sub.paymentMethod || 'Ví')}
                          </span>
                        </td>
                        <td style={{ textAlign: 'center' }}>
                          <StatusPill $status={isSubActive ? 'active' : (sub.status || 'expired')}>
                            {isSubActive ? <CheckCircle size={12} /> : <Clock size={12} />}
                            {isSubActive
                              ? (vi ? 'Đang kích hoạt' : 'Active')
                              : (sub.status === 'expired' ? (vi ? 'Đã hết hạn' : 'Expired') : sub.status)}
                          </StatusPill>
                        </td>
                      </tr>
                    );
                  ...
                  <tr>
                    <td colSpan={7} style={{ textAlign: 'center', padding: '36px', color: '#94a3b8' }}>
                      {isLoadingSubscriptions
                        ? (vi ? 'Đang tải lịch sử mua gói...' : 'Loading history...')
                        : (vi ? 'Bạn chưa mua gói dịch vụ nào.' : 'No package purchases found.')}
                    </td>
                  </tr>
                )}
              </tbody>
